...
public class CLI {
    ...
    public static void main(String[] args)
            throws IOException, NetworkException, MalformedResponseException,
                    org.json.simple.parser.ParseException, ServerRejectException,
                    ArgumentException {
        String ip;
        ...
        Option fpsArg =
                Option.builder()
                        ...
                        .hasArg()
                        .desc(
                                "FPS to run movie at. Ignored unless using `--action movie` and `--movie ...` is present")
                        .build();
        ...
        if (cmd.hasOption(
                "ip")) { // Everything requires an IP so if it's not there, don't try anything else
